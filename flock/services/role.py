...
def add(role, company_id):
    ...
    notify(u'{} added a new Role - <b>%s</b>' % role['name'], action='add')
...
def delete(role_id):
    ...
    notify(u'{} deleted a Role - <b>%s</b>' % role.name, action='delete')

def update(role):
    db.role_update(role)
    db.person_role_update(role)
    notify(u'{} edited a Role - <b>%s</b>' % role['name'], action='edit')
